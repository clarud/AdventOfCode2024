puzzleInput = """Button A: X+31, Y+16
Button B: X+35, Y+66
...
def inputParse(input):
    ...
    for lines in input.strip().split('\n'):
        parts = []
        if lines == "":
            machine.append(group.copy())
            group.clear()
            continue
        for item in lines.split(':')[1].strip().split(','):
            parts.append(int(item.strip()[2:]))
        group.append(parts)
    machine.append(group)
    return machine

def isWhole(number):
    return number % 1 == 0

def startMachine(machines):
    def findPath(A, B, prize):
        aCost = 3
        bCost = 1
        Ax, Ay = A
        Bx, By = B
        ...
        j = (Ax * prizey - Ay * prizex) / (Ax * By - Ay * Bx)
        print(i * aCost + j * bCost)
        if isWhole(i) and isWhole(j):
            return int(i * aCost + j * bCost)
        else:
            return 0
    ...
    for machine in machines:
        buttonA = (aRight, aforward) = (machine[0][0], machine[0][1])
        buttonB = (bRight, bforward) = (machine[1][0], machine[1][1])
        prize = (pRight, pForward) = (machine[2][0] + 10000000000000, machine[2][1] + 10000000000000)
        cost += findPath(buttonA, buttonB, prize)
    print(cost)
